...
export async function POST(request: Request) {
  try {
    ...
    if (!payload.email?.trim()) {
      return NextResponse.json({ error: "An email is required to send a verification message." }, { status: 400 });
    }
    ...
    db.prepare(
      `INSERT INTO verification_tokens (id, userId, token, createdAt, expiresAt)
       VALUES (?, ?, ?, ?, ?)
    ...
    return NextResponse.json({ error: "Unable to send the verification request." }, { status: 500 });
  }
}
